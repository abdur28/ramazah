...
- **Email** — the sender identity and the reminder cadence. Two addresses, both
  on the domain the sending provider has verified: `contact@` for invoices,
  codes and everything else transactional, and `news@` for newsletters. **Reply-to
  is `contact@`** — without it a reply to a newsletter goes to an address with no
  inbox behind it. Only `contact@` needs a forward at the registrar; a sending
