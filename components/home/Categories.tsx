...
export default function Categories() {
  ...
  return (
    <section 
      ref={sectionRef} 
      className="relative z-20 min-h-screen bg-wash rounded-t-3xl py-10 md:py-16 overflow-hidden"
    >
                  ...
                  >
                    ...
                    <div className="relative h-full flex flex-col justify-between">
                      <div className="flex-1 flex flex-col justify-end">
                        <h3 className={`font-heading text-3xl md:text-4xl lg:text-4xl ${category.textColor || ' text-foreground'} tracking-wide group-hover:translate-y-[-4px] transition-transform duration-300`}>
                          {category.title}
                        </h3>
                        {category.description && <p className={`font-body text-xs mt-2 ${category.textColor || 'text-foreground/90'} leading-relaxed line-clamp-3 max-w-xs`}>
                    ...
                    {index === 4 && (
                      <div className="absolute -right-10 -bottom-10 w-48 h-48 opacity-60 group-hover:opacity-80 transition-opacity">
                        <div className="relative w-full h-full">
                          {/* Replace with actual image */}
                          <div className="w-full h-full rounded-full bg-card/10 backdrop-blur-sm" />
                        </div>
                      </div>
                    )}
                  </WobbleCard>
